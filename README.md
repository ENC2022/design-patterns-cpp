...
# Construction & Execution - Basic Commands

The following commands can be executed in a Linux environment in order to generate the Makefiles, the binaries and run the examples

### Generating the MakeFiles (One time)
```
mkdir build
cd build
cmake ..
```
### Generating the binaries (One time)
```
make
```
### Executing the binaries (Many times)
```
./project.cpp
```
